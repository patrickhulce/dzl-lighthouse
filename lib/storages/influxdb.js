...
const schema = [
  {
    measurement: 'lhrs',
    tags: ['runId', 'label', 'url', 'hash'],
    fields: {
      runId: Influx.FieldType.STRING,
...
module.exports = {
  ...
  async run(lhrs, {runId, label, hash, storageOptions}) {
    ...
    const points = _(lhrs)
      ...
      .map((lhr, idx) => ({
        measurement: 'lhrs',
        tags: {runId, label, hash, url: lhr.url},
        fields: {runId, label, hash, ...lhr},
