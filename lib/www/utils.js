;(utils => {
  ...
  function getBoxAndWhiskerData(prop, opts = {}) {
    ...
    for (const [batchId, batch] of Object.entries(data)) {
      const allValues = []
      for (const [url, properties] of Object.entries(batch)) {
        if (where && !where({url, batch, batchId, properties})) continue
        allValues.push(...properties[prop])
      }
  ...
  function getHistogramData(prop) {
    const allValues = []
    for (const [url, properties] of Object.entries(data[currentBatchId])) {
      allValues.push(...properties[prop])
    }
