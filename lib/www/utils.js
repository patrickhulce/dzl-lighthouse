;(utils => {
  ...
  async function fetchData() {
    ...
    for (const [batchId, batch] of Object.entries(data)) {
      ...
      const date = batchId.replace(/^.*?-(\d{4})/, '$1')
      batch.metadata.date = new Date(date).toLocaleString()
  ...
  function renderEnvironment(opts = {}) {
    ...
    envEl.innerHTML = `
      <div class="row">
        ...
        <div class="col-4">Data Collected: <span>${date}</span></div>
      </div>
    `
  }
  ...
  function render({graphs = [], tiles = []}) {
    ...
    for (const [domId, dataFn, opts] of tiles) {
      ...
      el.classList.toggle('tile--warn', hasThresholds && isWarn && !isError)
      el.classList.toggle('tile--error', hasThresholds && !!isError)
      el.classList.toggle('tile--success', hasThresholds && !isWarn && !isError)
    }
