...
module.exports = async function serve(args) {
  ...
  app.get('/dashboard-data.json', async (req, res) => {
    const where = {
      label: req.query.label || 'local',
    }

    const [hashes, data] = await Promise.all([
      DataPoint.findAll({where, attributes: ['batchId', 'hash'], group: ['batchId', 'hash']}),
      DataPoint.findAll({
        where,
    ...
    const groups = _(data)
      ...
      .mapValues((values, batchId) => {
        ...
        dataByURL._metadata = _.find(hashes, {batchId})
