...
module.exports = async function collect(args) {
  const runId = `${args.label}-${new Date().toISOString()}`
  ...
  const options = {runId, hash, storage, lighthouseConfig, collectorOptions, storageOptions}
  ...
  const urls = config.collection.urls.slice()
