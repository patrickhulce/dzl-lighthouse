...
module.exports = {
  defaults: {
    ...
    reportsPath: path.join(__dirname, '../../.tmp/reports'),
  },
  ...
  async run({url, hash, storage, collectorOptions, storageOptions}) {
    const {cwd, configPath} = getPaths(collectorOptions)
    ...
    const lhr = JSON.parse(results.stdout)

    await storage.run({url, hash, lhr, storageOptions})
  },
}
